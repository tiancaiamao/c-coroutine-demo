...
#define vector(T) union { Vector v; T elem_type; }
...
#define vecAppend(ptr, value) \
    do { \
	    typeof((ptr)->elem_type) __tmp_t = (value);		       \
	    _vector_append(&(ptr)->v, &__tmp_t, sizeof((ptr)->elem_type)); \
    } while (0)

#define vecLen(ptr) ((ptr)->v.len)

#define vecGet(ptr, idx) \
    (((typeof((ptr)->elem_type) *)((ptr)->v.data))[idx])

#define vecPop(ptr) (((typeof((ptr)->elem_type) *)((ptr)->v.data))[--(ptr)->v.len])

#define vecSet(ptr, idx, value) \
	do {								\
		(((typeof((ptr)->elem_type) *)((ptr)->v.data))[idx] = value); \
			} while (0)

#define vecInit(ptr, initial_cap) \
    _vector_init(&(ptr)->v, (initial_cap), sizeof((ptr)->elem_type))
